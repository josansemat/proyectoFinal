...
function App() {
  ...
  useEffect(() => {
    ...
    console.log('Equipo:', currentTeam?.nombre, '| Color:', colorHex, '| ¿Fondo Claro?:', isLight);
  ...
  useEffect(() => {
    const loadSession = async () => {
      ...
      if (storedUserStr) {
        ...
        if (storedTeamStr) { setCurrentTeam(JSON.parse(storedTeamStr)); }
      }
    ...
    <Router>
      <div className="d-flex" style={{ minHeight: "100vh" }}>
        ...
        <div className="flex-grow-1 p-4 app-main-content">
          <Routes>
            <Route path="/" element={<Inicio user={user} team={currentTeam} onLogout={handleLogout} />} />
            <Route path="/inicio" element={<Inicio user={user} team={currentTeam} onLogout={handleLogout} />} />
            
            {/* --- NUEVAS RUTAS --- */}
            <Route path="/buscar-equipos" element={<BuscarEquipos user={user} userTeams={userTeams} />} />
            <Route path="/mi-club/solicitudes" element={<ManagerSolicitudes user={user} currentTeam={currentTeam} />} />
